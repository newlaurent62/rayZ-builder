...
class DataModel:
  ...
  def __init__(self, wizard):
    ...
    self.session_root = os.environ.get('RAY_SESSION_ROOT', '~/"Ray Sessions"')
...
class <xsl:value-of select='replace(upper-case(@id),"WIZARD","")'/>Wizard(QtWidgets.QWizard):
    ...
    def __init__(self, parent=None, templatedir='.', jsonfilename=None, startguioption=False, session-manager="ray_control"):
...
class <xsl:value-of select="last-page/@id"/>Page(BasePage):
    ...
    def validatePage(self):
      ...
      if not (datamodel.sessionNameAlreadyUsed()):
        ...
        if RaySessionTemplate().fillInTemplate(datamodelfile, templatedir, tmpdir, startgui=self.wizard().startgui, session_manager=self.wizard().session_manager) == 0:          
...
if __name__ == '__main__':
  ...
  import sys
  try:                                
    opts, args = getopt.getopt(sys.argv[1:], "hj:t:ds", ["help", "write-json-file=","template-dir=","debug","start-gui-option","session-manager="])
    print ("args list: ")
    print(opts)
  ...
  wizard = <xsl:value-of select='replace(upper-case(@id),"WIZARD","")'/>Wizard(templatedir=templatedir, jsonfilename=jsonfilename, startguioption=startguioption, session_manager=session_manager)
