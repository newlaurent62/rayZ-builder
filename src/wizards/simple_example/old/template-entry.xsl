...
class RaySessionTemplate:
  ...
  def fillInTemplate(self, datamodelfile, templatedir, outdir, fillonly=False, startgui=False, session_manager=None):
...
if __name__ == '__main__':
  ...
  import sys
  try:                                
    opts, args = getopt.getopt(sys.argv[1:], "hj:t:dfs", ["help", "read-json-file=","template-dir=","fill-only","start-gui","session-manager="])
    print ("optlist: ")
    print(opts)
  ...
  if RaySessionTemplate().fillInTemplate(datamodelfile, templatedir, tmpdir, fillonly=fillonly, startgui=startgui, session_manager=session_manager) == 0:          
